    ...
    <div class="flex flex-col items-center w-full min-h-screen p-4">
        <div class="flex flex-col md:flex-row w-full max-w-5xl gap-4">
            <div class="bg-white rounded-lg shadow-md p-6 w-full md:w-2/3">
                <form action="{{ route('payment.pay', $payment->trx) }}" method="POST" id="paymentForm">
                    @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        @forelse ($gateways as $gateway)
                            <div>
                                <input class="sr-only peer" type="radio" name="gateway"
                                ...
                                <label
                                    ...
                                    <div class="flex flex-col ltr:ml-6 rtl:mr-6">
                                        ...
                                        @if ($gateway->description)
                                            <span
                                                class="text-sm font-light text-gray-800">{{ $gateway->description }}</span>
                                        @endif
                                    </div>
                                </label>
                            </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6 w-full md:w-1/3">
                <div class="flex flex-col items-center">
                    ...
                    <div class="flex justify-between w-full mb-2">
                        ...
                        <span class="text-gray-800" id="feeAmount">0.0</span>
                    </div>
                    <div class="w-full border-t border-gray-200 my-4"></div>
                    <div class="flex justify-between w-full">
                        <span class="text-lg font-bold">{{ trans('filament-payments::messages.view.total') }}</span>
                        <span class="text-2xl font-bold text-black"
                            id="totalAmount">{{ Number::currency($payment->amount + $payment->charge, $payment->method_currency) }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                fetch('{{ route('payment.calculateFee') }}', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            console.error('Failed to fetch fee amount');
                        }
                    })
                    .catch(error => {
